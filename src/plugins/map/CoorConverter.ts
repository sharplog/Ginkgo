...
export default class CoorConverter {
  ...
  fromWGS84 (pos) {
    switch (this._coorStandard) {
      case Types.COOR_GCJ02:
        return wgs84_gcj02(pos)
      case Types.COOR_BD09:
        return wgs84_bd09(pos)
  ...
  toWGS84 (pos) {
    switch (this._coorStandard) {
      case Types.COOR_GCJ02:
        return gcj02_wgs84(pos)
      case Types.COOR_BD09:
        return bd09_wgs84(pos)
      case Types.COOR_WGS84:
        return pos
    }
  }
...
let _pi = Math.PI
let _xPi = Math.PI * 3000.0 / 180.0
  
/**
 * World Geodetic System ==> Mars Geodetic System
 */
function wgs84_gcj02 (wpos) {
    
  let wgLng = wpos.lng
  let wgLat = wpos.lat
  
  if (_outOfChina(wgLat, wgLng)) {
    return wpos
  }

  let dLat = _transformLat(wgLng - 105.0, wgLat - 35.0)
  let dLng = _transformLng(wgLng - 105.0, wgLat - 35.0)
 ...
 * gcj02 => baidu 09
 */
function gcj02_bd09 (gpos) {
      
  let x = gpos.lng
  let y = gpos.lat  
  let z = Math.sqrt(x * x + y * y) + 0.00002 * Math.sin(y * _xPi)  
  let theta = Math.atan2(y, x) + 0.000003 * Math.cos(x * _xPi)  
 ...
 */
function bd09_gcj02 (gpos) {
  let ggpos = gcj02_bd09(gpos)
  let dLat = ggpos.lat - gpos.lat
  let dLng = ggpos.lng - gpos.lng
  let wpos1 = { lat: gpos.lat - dLat, lng: gpos.lng - dLng }
  let gpos1 = gcj02_bd09(wpos1)
  let wpos2 = {lat: wpos1.lat - (gpos1.lat - gpos.lat), 
    lng: wpos1.lng - (gpos1.lng - gpos.lng)}
  let gpos2 = gcj02_bd09(wpos2)
 ...
 * wgs84 => baidu 09
 */
function wgs84_bd09 (wpos) {
  return gcj02_bd09(wgs84_gcj02(wpos))
}
    
/**
 * baidu 09 => wgs84
 */
function bd09_wgs84 (bpos) {
  return gcj02_wgs84(bd09_gcj02(bpos))
}
